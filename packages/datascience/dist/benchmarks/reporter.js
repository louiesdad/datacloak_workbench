...
class BenchmarkReporter {
    static generateReport(accuracyResults, performanceResults) {
        const timestamp = new Date().toISOString();
        const accuracyTestResults = accuracyResults.map(result => ({
    ...
    static exportToJson(report, filepath) {
    ...
    static exportToCsv(accuracyResults) {
        const headers = [
            'Test Name',
            'Accuracy',
            'Precision',
    ...
    static compareReports(currentReport, previousReport) {
        ...
        const currentAccuracyMap = new Map(currentReport.accuracyResults.map(r => [r.testName, r.metrics.accuracy]));
        const previousAccuracyMap = new Map(previousReport.accuracyResults.map(r => [r.testName, r.metrics.accuracy]));
        ...
        const currentPerfMap = new Map(currentReport.performanceResults.map(r => [r.name, r.avgExecutionTime]));
        const previousPerfMap = new Map(previousReport.performanceResults.map(r => [r.name, r.avgExecutionTime]));
    ...
    static calculateSummary(accuracyResults, performanceResults) {
    ...
    static generateRecommendations(accuracyResults, performanceResults) {
    ...
    static printAccuracyResults(results) {
    ...
    static printPerformanceResults(results) {
    ...
    static printRecommendations(recommendations) {
