...
export class PatternAnalyzer {
  private static readonly COMMON_PATTERNS: Array<{ name: string; pattern: RegExp }> = [
  ...
  static detectCustomPatterns(values: string[]): PatternMatch[] {
    const customPatterns: PatternMatch[] = [];
    const nonEmptyValues = values.filter(v => v && v.trim() !== '');
  ...
  private static detectLengthPattern(values: string[]): PatternMatch | null {
  ...
  private static detectPrefixPattern(values: string[]): PatternMatch | null {
